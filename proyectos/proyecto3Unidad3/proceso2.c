...
void init_control_mechanism()
{
    ...
    {
        perror("user 1 sem_open sem2write fails: ");
        exit(EXIT_FAILURE);
    }

    sem2read = sem_open("sem2read", O_CREAT, 0666, 0);
    if (sem2read == SEM_FAILED)
    {
        perror("user 1 sem_open sem2read fails: ");
        exit(EXIT_FAILURE);
    }
    ...
    {
        perror("user1 sem_open sem1write: ");
        exit(EXIT_FAILURE);
    }

    sem1read = sem_open("sem1read", 0);
    if (sem1read == SEM_FAILED)
    {
        perror("user1 sem_open sem1read: ");
        exit(EXIT_FAILURE);
    }
}
...
{
    shared_fd1 = shm_open("shm1", O_CREAT | O_RDWR, 0600);
    if (shared_fd1 < 0)
    {
        perror("Failed to create shared memory: ");
        exit(EXIT_FAILURE);
    }
    shared_fd2 = shm_open("shm0", O_RDONLY, 0600);
    if (shared_fd2 < 0)
    {
        perror("Failed to create shared memory: ");
        exit(EXIT_FAILURE);
...
void *sendMsg(void *parametros)
{
    ...
    {
        if (sem_post(sem2write) == -1)
        {
            perror("sem_post error sem2write: ");
            exit(EXIT_FAILURE);
        }
        ...
        if (sem_wait(sem1read) == -1)
        {
            perror("sem_wait error sem1read: ");
            exit(EXIT_FAILURE);
        }
    }
    if (sem_close(sem2write) == -1)
    {
        perror("sem_close fails sem2write: ");
        exit(EXIT_FAILURE);
    }

    if (sem_unlink("sem2write") == -1)
    {
        perror("sem_unlink falls sem2write: ");
        exit(EXIT_FAILURE);
    }

    if (sem_close(sem2read) == -1)
    {
        perror("sem_close fails sem2read: ");
        exit(EXIT_FAILURE);
    }

    if (sem_unlink("sem2read") == -1)
    {
        perror("sem_unlink falls sem2read: ");
        exit(EXIT_FAILURE);
    }
...
void *reciveMsg(void *parametros)
{
    ...
    for (;;)
    {
        if (sem_wait(sem1write) == -1)
        {
            perror("sem_wait error sem1write: ");
            exit(EXIT_FAILURE);
        }
